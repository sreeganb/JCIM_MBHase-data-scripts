...
for i in range(m):
    ...
    atom1 = u1.select_atoms("resname QM and name O4")
    atom2 = u1.select_atoms("resname QM and name H1''")
    dist1 = get_dist(atom1, atom2)
    disoh.append(float(dist1[2]))
    atom1 = u1.select_atoms("resname QM and name OH2'")
    atom2 = u1.select_atoms("resname QM and name H5")
    dist1 = get_dist(atom1, atom2)
    disoh1.append(float(dist1[2]))
    atom1 = u1.select_atoms("resname QM and name OH2'")
    atom2 = u1.select_atoms("resname QM and name H1''")
    dist1 = get_dist(atom1, atom2)
    disoh2.append(float(dist1[2]))
plt.plot(disco, label = r'dis(C8-H5)', linewidth = 1)
plt.plot(disoh, label = r'dis(O4-H1'')', linewidth = 1)
plt.plot(disoh1, label = r'dis(OH2-H5)', linewidth = 1)
plt.plot(disoh2, label = r'dis(OH2-H1)', linewidth = 1)
